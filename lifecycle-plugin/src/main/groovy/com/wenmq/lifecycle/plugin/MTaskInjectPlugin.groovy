package com.wenmq.lifecycle.plugin
...
class MTaskInjectPlugin extends Transform implements Plugin<Project> {
    ...
    void transform(@NonNull TransformInvocation transformInvocation) {
        println '--------------- TaskInjectPlugin visit start --------------- '
        ...
        if (jarInput.file.getAbsolutePath().endsWith(".jar")) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (checkClassFile(entryName)) {
                    ...
                    jdk.internal.org.objectweb.asm.ClassReader classReader = new jdk.internal.org.objectweb.asm.ClassReader(IOUtils.toByteArray(inputStream))
                    ClassWriter classWriter = new ClassWriter(classReader, ClassWriter.COMPUTE_MAXS)
                    ClassVisitor cv = new LifecycleClassVisitor(classWriter)
                    classReader.accept(cv, EXPAND_FRAMES)
                    byte[] code = classWriter.toByteArray()
...
//        if ("android/support/v4/app/FragmentActivity" == p) return true;
        return false;
